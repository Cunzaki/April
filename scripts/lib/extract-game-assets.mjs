...
/** MeshId thumbnails from ReplicatedStorage.Benches / VMs / Armors (game prefab icons). */
export function parsePrefabMeshes(tsvPath) {
  ...
  for (const line of fs.readFileSync(tsvPath, "utf8").split("\n").slice(1)) {
    ...
    const mesh = rbxId(parts[2]);
    if (!instPath || !mesh) continue;

    let m = instPath.match(/ReplicatedStorage\.Benches\.([^.]+)\.([^.]+)/);
    if (m) {
      updatePick(pick, m[1], mesh, benchPartScore(m[2], instPath), m[2]);
      continue;
    }

    m = instPath.match(/ReplicatedStorage\.VMs\.([^.]+)\.Default\.Weapon\.([^.]+)/);
    if (m) {
      updatePick(pick, m[1], mesh, vmPartScore(m[2], instPath), m[2]);
      continue;
    }

    m = instPath.match(/ReplicatedStorage\.Armors\.([^.]+)\.Default\.(?:[^.]+\.)*([^.]+)$/);
    if (m) {
      updatePick(pick, m[1], mesh, armorPartScore(m[2]), m[2]);
    }
  }
  return pick;
}
...
export function parseAttachmentTextures(tsvPath) {
  ...
  // ponytail: icon = MeshId thumbnail (3D preview), not TextureID (UV unwrap sheet)
